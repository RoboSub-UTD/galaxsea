...
class ObstacleTypeMapNode(Node):
    def __init__(self):
        ...
        self.marker_pub = self.create_publisher(
            MarkerArray, '/obstacle_type_markers', 10)
 
        rate = self.get_parameter('publish_rate').get_parameter_value().double_value
        self.create_timer(1.0 / rate, self.publish_markers)
 
        self.get_logger().info(
            f'ObstacleTypeMapNode started — '
            f'{len(self.obstacles)} obstacles loaded from {self.save_path}')

 
    def add_obstacle(self, x: float, y: float, label: str) -> str:
        for key, obs in self.obstacles.items():
            if obs['label'] == label:
                dx = obs['x'] - x
                dy = obs['y'] - y
                if (dx ** 2 + dy ** 2) ** 0.5 <= self.dedup_radius:
                    self.obstacles[key]['x'] = x
                    self.obstacles[key]['y'] = y
                    self.save_to_disk()
                    return key
 
        key = f"{label}_{round(x, 2)}_{round(y, 2)}"
        self.obstacles[key] = {'x': x, 'y': y, 'label': label}
        self.save_to_disk()
        self.get_logger().info(f"Added obstacle type '{label}' at ({x:.2f}, {y:.2f})")
        return key
    ...
    def publish_markers(self):
        ...
        for i, (key, obs) in enumerate(self.obstacles.items()):
            ...
            sphere.ns = 'obstacle_type_spheres'
            sphere.id = i
